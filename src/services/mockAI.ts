...
const KNOWLEDGE_BASE: Record<string, MockResponse[]> = {
  ...
  default: [
    {
      content: 'Can you be more specific? I can help with project management, risk assessment, or contract drafting.',
      chips: ['Risk Analysis', 'Draft Contract'],
    },
    { content: "I'm Cortex, your AI assistant. How can I help optimizing your workflow today?" },
  ],
  hello: [{ content: 'Hello! Ready to help you build the future of work.', chips: ['Create Project', 'Check Status'] }],

  // Project Creation Context
  project_create_requirements: [
    {
      content:
        '根据您的输入，我已生成结构化需求文档。点击下方按钮可自动填充表单：\n\n**项目名称**: NextGen 出行平台\n**核心功能**: GPS追踪、支付集成、匹配算法\n**预算范围**: $50,000 - $80,000',
      chips: ['调整预算', '更换技术栈'],
      action: {
        type: 'autofill_form',
        label: '✨ 自动填充表单',
  ...
  project_create_budget: [
    {
      content:
        "For a full-scale ride-sharing app with real-time features, a budget of $5k is too low. Market average is between $40k - $100k for MVP. I suggest creating a 'Prototype' phase first if budget is tight.",
      chips: ['Adjust to $50k', 'Keep $5k (Risky)'],
    },
  ],
...
export class MockAIService {
  static async generateResponse(userMessage: string, context: PageContextData): Promise<AIChatMessage> {
    ...
    if (context.type === 'project_create') {
      if (
        ...
        response = {
          content:
            "I can help you draft the project description or estimate the budget. Try asking 'Help me write requirements for a ride app'.",
        }
      }
      ...
      if (lowerMsg.includes('risk') || lowerMsg.includes('status')) {
        ...
        response = {
          content: `I'm monitoring project "${context.data?.title || 'Current Project'}". Ask me about risks, budget, or milestones.`,
        }
      }
    }
    // 2. Global Keywords
