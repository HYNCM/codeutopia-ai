...
export const ActionCard: React.FC<ActionCardProps> = ({ action, onExecute }) => {
  const getIcon = () => {
    switch (action.type) {
      case 'autofill_form': return <FileText className="w-5 h-5 text-indigo-600" />;
      case 'highlight_risk': return <AlertTriangle className="w-5 h-5 text-orange-600" />;
  ...
  const getColor = () => {
    switch (action.type) {
      case 'autofill_form': return 'bg-indigo-50 border-indigo-200 hover:bg-indigo-100';
      case 'highlight_risk': return 'bg-orange-50 border-orange-200 hover:bg-orange-100';
      default: return 'bg-purple-50 border-purple-200 hover:bg-purple-100';
    }
  };

  return (
    <div className={`mt-3 p-4 rounded-xl border transition-all cursor-pointer group ${getColor()}`}
