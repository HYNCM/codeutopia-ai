...
const PENDING_TASKS = [
  { id: 't1', type: 'application', title: '新人才申請 (3)', description: '王建國申請參與智能客服項目', time: '2小時前', priority: 'high' },
  { id: 't2', type: 'milestone', title: '里程碑驗收 (1)', description: '「代碼開發里程碑」待驗收', time: '5小時前', priority: 'medium' },
  { id: 't3', type: 'message', title: '人才溝通回覆', description: '李美華詢問設計需求細節', time: '1天前', priority: 'low' },
  { id: 't4', type: 'ai', title: 'AI 成果審核 (2)', description: 'UX設計師AI輸出待確認', time: '1天前', priority: 'medium' },
  { id: 't5', type: 'payment', title: '里程碑結算', description: '「需求確認里程碑」待付款', time: '2天前', priority: 'high' },
];
            ...
            <div className="col-span-12 lg:col-span-8 space-y-6">
              ...
              <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
                ...
                <div className="grid grid-cols-4 md:grid-cols-7 gap-3">
                  {[
                    { icon: Plus, label: '發布項目', color: 'bg-blue-500', path: '/projects/new' },
                    ...
                    { icon: DollarSign, label: '費用結算', color: 'bg-emerald-500', action: () => setActiveTab('finance') },
                  ].map((item, idx) => (
                    <button
                      key={idx}
                      onClick={item.action}
                    ...
                    >
                      <div className={`w-10 h-10 ${item.color} rounded-xl flex items-center justify-center mx-auto mb-2 group-hover:scale-110 transition-transform`}>
              ...
              <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  {[
                    { label: '進行中項目', value: DASHBOARD_STATS.activeProjects, icon: Briefcase, color: 'text-blue-600', bg: 'bg-blue-100' },
                    { label: '對接人才數', value: DASHBOARD_STATS.connectedTalents, icon: Users, color: 'text-green-600', bg: 'bg-green-100' },
                    { label: 'AI 調用次數', value: DASHBOARD_STATS.aiAssistCalls, icon: Brain, color: 'text-purple-600', bg: 'bg-purple-100' },
                    { label: '總支出', value: `$${(DASHBOARD_STATS.totalSpent / 1000).toFixed(0)}K`, icon: DollarSign, color: 'text-orange-600', bg: 'bg-orange-100' },
                  ].map((stat, idx) => (
              ...
              <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
                ...
                <div className="space-y-4">
                  {ACTIVE_PROJECTS.map((project) => (
                    <div key={project.id} className="p-4 bg-gray-50 rounded-xl hover:bg-gray-100 transition-colors cursor-pointer">
                      <div className="flex items-start justify-between mb-3">
                        <div>
                          ...
                          <div className="flex items-center space-x-4 mt-1 text-sm text-gray-500">
                            ...
                            <span className="flex items-center space-x-1">
                              ...
                              <span>{project.connectedTalents} 人才</span>
                            </span>
                            <span className="flex items-center space-x-1">
                        ...
                        <div className="flex items-center space-x-2">
                          {project.aiAssistants.map((ai) => (
                            <span key={ai} className="px-2 py-1 bg-purple-100 text-purple-700 rounded text-xs">
                              {ai === 'frontend_engineer' ? '前端 AI' : ai === 'ux_designer' ? '設計 AI' : ai === 'qa_engineer' ? '測試 AI' : ai}
                            </span>
                          ))}
                        </div>
                      </div>
              ...
              <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <span className="px-2 py-1 bg-red-100 text-red-700 rounded text-sm">{PENDING_TASKS.length} 項待處理</span>
                </div>
                <div className="space-y-3">
                  {PENDING_TASKS.map((task) => (
                    <div key={task.id} className="flex items-center space-x-4 p-3 hover:bg-gray-50 rounded-xl transition-colors cursor-pointer">
                      <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${
                        task.type === 'application' ? 'bg-blue-100' :
                        task.type === 'milestone' ? 'bg-green-100' :
                        task.type === 'message' ? 'bg-orange-100' :
                        task.type === 'ai' ? 'bg-purple-100' : 'bg-emerald-100'
                      }`}>
                        {task.type === 'application' ? <Users className="w-5 h-5 text-blue-600" /> :
                         task.type === 'milestone' ? <Target className="w-5 h-5 text-green-600" /> :
                         task.type === 'message' ? <MessageSquare className="w-5 h-5 text-orange-600" /> :
                         task.type === 'ai' ? <Brain className="w-5 h-5 text-purple-600" /> :
              ...
              <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
                ...
                <div className="space-y-4">
                  {RECOMMENDED_TALENTS.map((talent) => (
                    <div key={talent.id} className="p-3 bg-gray-50 rounded-xl hover:bg-gray-100 transition-colors cursor-pointer">
                    ...
                    {ACTIVE_PROJECTS.map((project) => (
                      <tr key={project.id} className="hover:bg-gray-50 transition-colors">
                        ...
                        <td className="px-6 py-4">
                          ...
                          <div className="text-sm text-gray-500">已花 ${project.spent.toLocaleString()}</div>
                        </td>
                        <td className="px-6 py-4">
                          <div className="flex items-center space-x-1">
                            <Users className="w-4 h-4 text-gray-400" />
                            <span className="text-gray-900">{project.connectedTalents}</span>
                          </div>
                        </td>
                        <td className="px-6 py-4">
                          <div className="flex space-x-1">
                            {project.aiAssistants.map((ai) => (
                              <span key={ai} className="px-2 py-0.5 bg-purple-100 text-purple-700 rounded text-xs">
                                AI
                              </span>
                            ))}
                          </div>
                        </td>
                        <td className="px-6 py-4 text-sm text-gray-500">{project.lastActivity}</td>
        ...
        {activeTab === 'talents' && (
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {RECOMMENDED_TALENTS.map((talent) => (
                <div key={talent.id} className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 hover:shadow-md transition-shadow">
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
              ...
              <div className="overflow-x-auto">
                <table className="w-full">
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {[
                      { project: '智能客服系統開發', milestone: '代碼開發里程碑', amount: '$15,000', status: '已完成', date: '2024-01-15' },
                      { project: '數據分析平台 UI/UX 優化', milestone: '設計調研里程碑', amount: '$5,600', status: '已完成', date: '2024-01-10' },
                      { project: '智能客服系統開發', milestone: '需求確認里程碑', amount: '$9,000', status: '已完成', date: '2024-01-05' },
                    ].map((row, idx) => (
