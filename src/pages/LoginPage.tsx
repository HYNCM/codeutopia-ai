          ...
          {showTestAccounts && (
            <div className='absolute right-0 mt-2 w-80 bg-gray-800/95 backdrop-blur-xl rounded-xl p-4 shadow-2xl border border-gray-700'>
              <div className='flex items-center justify-between mb-3'>
                ...
                <button onClick={() => setShowTestAccounts(false)} className='text-gray-400 hover:text-white'>
                  ×
                </button>
              </div>
                ...
                {Object.keys(TRANSLATIONS).map((lang) => (
                  <button
                    ...
                    className={`py-1 px-2 rounded-lg text-xs font-medium transition-colors ${
                      language === lang ? 'bg-purple-600 text-white' : 'bg-gray-700 text-gray-300 hover:bg-gray-600'
                    }`}>
                    {lang.toUpperCase()}
                  </button>
                ))}
                  ...
                  {TEST_ACCOUNTS.map((account, idx) => (
                    <button
                      ...
                      <div className='flex items-center justify-between'>
                        ...
                        <span
                          className={`text-xs px-2 py-0.5 rounded ${account.role === 'webadmin' ? 'bg-red-500/20 text-red-400' : account.role === 'regional_manager' ? 'bg-purple-500/20 text-purple-400' : account.role === 'project_initiator' ? 'bg-blue-500/20 text-blue-400' : 'bg-green-500/20 text-green-400'}`}>
                          {account.roleName}
                        </span>
                      </div>
          ...
          <div className='flex bg-gray-800/50 rounded-xl p-1 mb-6'>
            <button
              ...
              className={`flex-1 py-2.5 px-4 rounded-lg text-sm font-medium transition-all ${
                activeTab === 'login' ? 'bg-purple-600 text-white' : 'text-gray-400 hover:text-white'
              }`}>
              {t.login}
            </button>
            <button
              ...
              className={`flex-1 py-2.5 px-4 rounded-lg text-sm font-medium transition-all ${
                activeTab === 'register' ? 'bg-purple-600 text-white' : 'text-gray-400 hover:text-white'
              }`}>
              {t.register}
            </button>
          </div>
              ...
              <div className='grid grid-cols-4 gap-3'>
                {[
                  ...
                  <button
                    ...
                    <span
                      className={`w-5 h-5 rounded-full ${social.color} flex items-center justify-center text-xs font-bold ${
            ...
            <form
              ...
              <div className='pt-2'>
                <label className='flex items-start space-x-3 text-sm text-gray-400'>
                  <input
                    type='checkbox'
                    className='w-4 h-4 mt-0.5 rounded border-gray-600 bg-gray-700 text-purple-500 focus:ring-purple-500'
                  />
                  <span>
                    我已阅读并同意
                    <a href='#' className='text-purple-400 hover:text-purple-300'>
