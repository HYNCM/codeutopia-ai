...
import {
  DollarSign, CreditCard, Building2, Wallet, ArrowUpRight, ArrowDownRight,
  CheckCircle, Clock, AlertTriangle, FileText, Briefcase, Users,
  Globe, Calendar, Search, Filter, MoreVertical, Download, Upload,
  RefreshCw, ChevronRight, X, Send, Shield, Zap, TrendingUp,
  History, PieChart, BarChart3, ArrowLeftRight, ExternalLink
} from 'lucide-react';
...
const PaymentSettlementPage: React.FC = () => {
  ...
  const filteredPayments = paymentRecords.filter(record => {
    const matchesSearch = record.projectTitle.toLowerCase().includes(searchQuery.toLowerCase()) ||
        ...
        {activeTab === 'overview' && (
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              {[
                { label: '总收入', value: formatCurrency(stats.totalReceived, 'USD'), change: '+12.5%', icon: ArrowUpRight, color: 'from-green-500 to-emerald-500' },
                { label: '总支出', value: formatCurrency(stats.totalSpent, 'USD'), change: '+8.3%', icon: ArrowDownRight, color: 'from-red-500 to-orange-500' },
                { label: '待付款', value: formatCurrency(stats.pendingPayments, 'USD'), change: '-5.2%', icon: Clock, color: 'from-yellow-500 to-orange-500' },
                { label: '托管余额', value: formatCurrency(stats.escrowBalance, 'USD'), change: '+15.8%', icon: Shield, color: 'from-purple-500 to-pink-500' },
              ].map((stat, index) => (
                    ...
                    <div key={payment.id} className="flex items-center justify-between p-3 bg-slate-700/30 rounded-lg">
                      <div className="flex items-center space-x-3">
                        <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${
                          payment.type === 'milestone_release' ? 'bg-green-500/20' :
                          payment.type === 'milestone_escrow' ? 'bg-blue-500/20' : 'bg-purple-500/20'
                        }`}>
                      ...
                      <div className="text-right">
                        <div className={`font-medium ${
                          payment.type === 'milestone_release' || payment.type === 'bonus' ? 'text-green-400' :
                          payment.type === 'refund' ? 'text-orange-400' : 'text-white'
                        }`}>
                ...
                {escrowAccounts.map((account) => (
                  <div key={account.id} className="p-4 bg-slate-700/30 rounded-lg">
                    ...
                    <div className="flex items-center justify-between mb-3">
                      ...
                      <span className="text-white font-medium">{formatCurrency(account.totalAmount, account.currency)}</span>
                    </div>
                    <div className="flex items-center justify-between mb-3">
                      <span className="text-gray-400 text-sm">已释放</span>
                      <span className="text-green-400 font-medium">{formatCurrency(account.releasedAmount, account.currency)}</span>
                    </div>
                      ...
                      <tr key={payment.id} className="border-b border-slate-700/50 hover:bg-slate-700/30">
                        ...
                        <td className="py-4">
                          <span className={`text-xs px-2 py-1 rounded-full ${getTypeColor(payment.type)}`}>
                            {payment.type === 'milestone_release' ? '释放' :
                             payment.type === 'milestone_escrow' ? '托管' :
                             payment.type === 'bonus' ? '奖励' : '退款'}
                          </span>
                        </td>
                        <td className="py-4">
                          <div className={`font-medium ${
                        ...
                        <td className="py-4">
                          <span className={`text-xs px-2 py-1 rounded-full ${getStatusColor(payment.status)}`}>
                            {payment.status === 'completed' ? '已完成' :
                             ...
                             payment.status === 'processing' ? '处理中' : payment.status}
                          </span>
                        </td>
                ...
                <div key={account.id} className="bg-slate-800/50 rounded-xl p-6 border border-purple-500/20">
                  ...
                  <div className="grid grid-cols-3 gap-4 mb-6">
                    <div className="bg-slate-700/30 rounded-lg p-4 text-center">
                      <div className="text-2xl font-bold text-white">{formatCurrency(account.totalAmount, account.currency)}</div>
                      <div className="text-gray-400 text-sm">总托管金额</div>
                    </div>
                    <div className="bg-green-500/10 rounded-lg p-4 text-center">
                      <div className="text-2xl font-bold text-green-400">{formatCurrency(account.releasedAmount, account.currency)}</div>
                      <div className="text-gray-400 text-sm">已释放</div>
                    </div>
                    <div className="bg-blue-500/10 rounded-lg p-4 text-center">
                      <div className="text-2xl font-bold text-blue-400">{formatCurrency(account.pendingAmount, account.currency)}</div>
                  ...
                  <div>
                    ...
                    <div className="space-y-2">
                      {account.milestones.map((milestone) => (
                        <div key={milestone.milestoneId} className="flex items-center justify-between p-3 bg-slate-700/30 rounded-lg">
                          <div className="flex items-center space-x-3">
                          ...
                          <div className="flex items-center space-x-4">
                            ...
                            <span className={`text-xs px-2 py-1 rounded-full ${getStatusColor(milestone.status)}`}>
                              {milestone.status === 'released' ? '已释放' :
                               milestone.status === 'locked' ? '已锁定' : '待处理'}
                            </span>
                          </div>
                        </div>
                      ))}
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-4">
                {currencyRates.map((rate) => (
                  <div key={rate.code} className="bg-slate-700/30 rounded-lg p-4 text-center">
                    ...
                    <div className="text-xl font-bold text-purple-400">{rate.symbol}{(1 / rate.rateToUSD).toFixed(4)}</div>
        ...
        {activeTab === 'disputes' && (
          <div className="space-y-6">
            ...
            <div className="bg-slate-800/50 rounded-xl p-6 border border-purple-500/20">
              ...
              <div className="space-y-4">
                {[
                  { id: 'DSP001', project: '跨境电商平台开发', milestone: '前端开发', amount: '$8,500', status: '待处理', reason: '交付物未达标准' },
                  { id: 'DSP002', project: 'AI智能客服系统', milestone: '模型训练', amount: '$12,000', status: '处理中', reason: '进度延期争议' },
                ].map((dispute, index) => (
