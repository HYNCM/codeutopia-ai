...
export default function ContractorDashboard({ currentRole }: ContractorDashboardProps = {}) {
  ...
  // 計算總體進度
  const totalProgress = Math.round(TASKS.reduce((acc, task) => acc + task.progress, 0) / TASKS.length)
    ...
    <div className='min-h-screen bg-gray-900 text-white'>
      ...
      <div className='flex'>
        ...
        <aside className='w-64 bg-gray-800/30 border-r border-gray-700 min-h-[calc(100vh-4rem)] hidden md:block'>
          <div className='p-4'>
            <div className='text-xs font-semibold text-gray-500 uppercase tracking-wider mb-3'>專案頻道</div>
            <div className='space-y-1'>
              {CHANNELS.map((channel) => (
                <button
                  ...
                  {channel.unread && <span className='w-2 h-2 bg-red-500 rounded-full' />}
                </button>
              ))}
            </div>
            ...
            {/* 虛擬團隊 */}
            <div className='mt-8'>
              <div className='text-xs font-semibold text-gray-500 uppercase tracking-wider mb-3'>團隊成員</div>
              <div className='space-y-2'>
                {TEAM_MEMBERS.map((member) => (
                  <div key={member.id} className='flex items-center space-x-3 px-3 py-2 rounded-lg bg-gray-700/30'>
            ...
            {/* 工具整合 */}
            <div className='mt-8'>
              <div className='text-xs font-semibold text-gray-500 uppercase tracking-wider mb-3'>整合服務</div>
              <div className='grid grid-cols-2 gap-2'>
                {TOOLS.map((tool) => (
                  <div key={tool.name} className='flex items-center space-x-2 px-3 py-2 bg-gray-700/30 rounded-lg'>
            ...
            {/* 開發模式 */}
            <div className='mt-8'>
              <div className='text-xs font-semibold text-gray-500 uppercase tracking-wider mb-3'>開發模式</div>
              <div className='space-y-2'>
                {DEV_MODES.map((mode) => (
                  <button
                    key={mode.id}
        ...
        <main className='flex-1 p-6'>
          ...
          <div className='grid grid-cols-2 md:grid-cols-4 gap-4 mb-6'>
            {[
              ...
              {
                label: '總收益',
                value: `$${(CONTRACTOR_STATS.totalEarnings / 1000).toFixed(0)}K`,
                icon: DollarSign,
                color: 'purple',
              },
            ].map((stat, idx) => (
            ...
            <div className='col-span-12 lg:col-span-8 space-y-6'>
              ...
              <div className='bg-gray-800/50 backdrop-blur-sm rounded-2xl border border-gray-700 p-6'>
                ...
                <div className='space-y-4'>
                  {TASKS.map((task) => (
                    <div key={task.id} className='p-4 bg-gray-700/30 rounded-xl border border-gray-700'>
                  ...
                  {DEPLOYMENT_PIPELINE.map((stage, idx) => (
                    <div key={stage.stage} className='flex items-center'>
                      ...
                      {idx < DEPLOYMENT_PIPELINE.length - 1 && (
                        <div
                          className={`w-0.5 h-8 ml-4 ${stage.status === 'completed' ? 'bg-green-500' : 'bg-gray-700'}`}
                        />
                      )}
                    </div>
                  ))}
