...
const RegionalManagerDashboard: React.FC<RegionalManagerDashboardProps> = ({ currentRole }) => {
  const [activeTab, setActiveTab] = useState<'overview' | 'projects' | 'users' | 'reports' | 'disputes' | 'ai_config'>(
    'overview',
  )
  ...
  const regions: RegionalData[] = [
    ...
    },
    {
      regionId: 'EMEA',
      regionName: '欧洲中东非洲',
      regionNameEn: 'Europe, Middle East & Africa',
      countryCount: 52,
      activeUsers: 8920,
      activeProjects: 654,
      revenue: 3280000,
  ...
  const aiSchedulingRequests = [
    {
      id: 1,
      projectId: 'PRJ001',
      requestType: 'additional_ai',
      aiRole: 'UX Designer AI',
      reason: '原型设计需要多轮迭代',
      status: 'pending',
  ...
  const regionalDisputes = [
    {
      id: 'DISP001',
      projectName: '跨境电商平台开发',
      initiator: 'ABC Corp',
      contractor: 'TechFreelancer',
      amount: 45000,
      status: 'preliminary',
      region: 'APAC',
    },
    {
      ...
      initiator: 'StartupXYZ',
      contractor: 'DesignPro',
      amount: 12000,
      status: 'escalated',
      region: 'AMERS',
    },
                ...
                <tbody className='text-white text-sm'>
                  {regionalDisputes.map((dispute) => (
                    <tr key={dispute.id} className='border-b border-[var(--border-color)]/50 hover:bg-[var(--bg-input)]/30'>
                      ...
                      <td className='py-4'>
                        <button className='text-purple-400 hover:text-purple-300 text-xs'>处理争议</button>
                      </td>
                    </tr>
                  ))}
                </tbody>
