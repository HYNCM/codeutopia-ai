...
const PENDING_REVIEWS = [
  { id: 'p1', type: 'project', title: '企業內部管理系統開發', submitter: '創新科技有限公司', submittedAt: '2024-01-18', status: 'pending', priority: 'high' },
  { id: 'p2', type: 'user', title: '接案者資質認證', submitter: '李美華', submittedAt: '2024-01-18', status: 'pending', priority: 'medium' },
  { id: 'p3', type: 'project', title: 'AI 驅動數據分析平台', submitter: '數據智慧有限公司', submittedAt: '2024-01-17', status: 'pending', priority: 'low' },
];

const DISPUTES = [
  { id: 'd1', project: '智能客服系統開發', type: '里程碑驗收爭議', status: '初步處理中', initiator: '接案者', amount: '$3,000', createdAt: '2024-01-16' },
  { id: 'd2', project: '電商平台前端開發', type: '交付質量爭議', status: '待裁定', initiator: '項目主理人', amount: '$5,500', createdAt: '2024-01-15' },
];
                          ...
                          <tr key={region.id} className="hover:bg-gray-750">
                            <td className="py-4">
                              <div className="flex items-center space-x-2">
                                <span className="text-lg">{region.id === 'tw' ? '🇹🇼' : region.id === 'sg' ? '🇸🇬' : region.id === 'jp' ? '🇯🇵' : region.id === 'hk' ? '🇭🇰' : region.id === 'my' ? '🇲🇾' : '🇦🇺'}</span>
              ...
              <div className="col-span-12 lg:col-span-4 space-y-6">
                ...
                <div className="bg-gray-800 rounded-2xl p-6 border border-gray-700">
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <span className="px-2 py-1 bg-amber-500/20 text-amber-400 rounded text-sm">{PENDING_REVIEWS.length}</span>
                  </div>
                  <div className="space-y-3">
                    {PENDING_REVIEWS.map((item) => (
                      <div key={item.id} className="p-3 bg-gray-700/50 rounded-xl hover:bg-gray-700 transition-colors cursor-pointer">
                        <div className="flex items-start justify-between mb-2">
                          <div className="flex items-center space-x-2">
                            {item.type === 'project' ? <Briefcase className="w-4 h-4 text-blue-400" /> : <Users className="w-4 h-4 text-green-400" />}
                    ...
                    {RECENT_ACTIVITIES.map((activity, idx) => (
                      <div key={idx} className="flex items-start space-x-3">
                        <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${
                          ...
                          activity.type === 'dispute' ? 'bg-amber-500/20' : 'bg-pink-500/20'
                        }`}>
                          <activity.icon className={`w-4 h-4 ${
                            activity.type === 'user' ? 'text-blue-400' :
                            activity.type === 'project' ? 'text-green-400' :
                            activity.type === 'payment' ? 'text-purple-400' :
                            activity.type === 'dispute' ? 'text-amber-400' : 'text-pink-400'
                          }`} />
                        </div>
          ...
          <div className="space-y-6">
            ...
            <div className="bg-gray-800 rounded-2xl border border-gray-700 overflow-hidden">
              <table className="w-full">
                ...
                <tbody className="divide-y divide-gray-700">
                  {[
                    { name: '張偉明', role: '項目主理人', company: '創新科技有限公司', status: '正常', joined: '2024-03-15', projects: 12, spent: 285000 },
                    { name: '王建國', role: '接案者', skills: '全棧工程師', status: '正常', joined: '2024-02-20', projects: 28, earned: 158000 },
                    { name: '李美華', role: '接案者', skills: 'UX 設計師', status: '正常', joined: '2024-01-10', projects: 34, earned: 186000 },
                    { name: '陳志偉', role: '區域主理人', region: '日本區', status: '正常', joined: '2023-12-01', projects: 89, revenue: 245000 },
                  ].map((user, idx) => (
                    <tr key={idx} className="hover:bg-gray-700/50">
                      ...
                      <td className="px-6 py-4">
                        <span className={`px-2.5 py-1 rounded-full text-xs font-medium ${
                          user.role.includes('管理員') ? 'bg-red-500/20 text-red-400' :
                          user.role.includes('主理人') ? 'bg-purple-500/20 text-purple-400' :
                          user.role.includes('項目') ? 'bg-blue-500/20 text-blue-400' :
                      ...
                      <td className="px-6 py-4 text-sm text-gray-300">
                        {user.projects} 項目 / ${(user.spent || user.earned || user.revenue) > 1000 ? (user.spent || user.earned || user.revenue) / 1000 + 'K' : user.spent || user.earned || user.revenue}
                      </td>
                  ...
                  <div key={project.id} className="p-4 bg-gray-700/50 rounded-xl">
                    <div className="flex items-start justify-between mb-3">
                      <div>
                        ...
                        <p className="text-sm text-gray-400">提交者：{project.submitter} • {project.submittedAt}</p>
                      </div>
                      <div className="flex items-center space-x-2">
                  ...
                  <div key={dispute.id} className="p-6 hover:bg-gray-700/50 transition-colors">
                    <div className="flex items-start justify-between mb-3">
                      <div>
                        ...
                        <p className="text-sm text-gray-400">{dispute.type} • 發起人：{dispute.initiator}</p>
                      </div>
            ...
            <div className="bg-gray-800 rounded-2xl p-6 border border-gray-700">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  ...
                  <div className="flex items-center space-x-4">
                    <div className="flex-1">
                      ...
                      <input type="number" defaultValue="10" className="w-full mt-1 py-2 px-3 bg-gray-700 border border-gray-600 rounded-lg text-white" />
                    </div>
                    <div className="flex-1">
                      <span className="text-sm text-gray-500">接案者</span>
                      <input type="number" defaultValue="5" className="w-full mt-1 py-2 px-3 bg-gray-700 border border-gray-600 rounded-lg text-white" />
                    </div>
                  </div>
                </div>
                <div>
                  ...
                  <div className="flex items-center space-x-4">
                    <input type="number" defaultValue="0.5" className="flex-1 py-2 px-3 bg-gray-700 border border-gray-600 rounded-lg text-white" />
                ...
                <div>
                  ...
                  <div className="flex items-center space-x-4">
                    <input type="number" defaultValue="20" className="flex-1 py-2 px-3 bg-gray-700 border border-gray-600 rounded-lg text-white" />
                    <span className="text-gray-400">%</span>
                  </div>
                </div>
                <div>
                  <label className="block text-sm text-gray-400 mb-2">結算時效</label>
                  <input type="number" defaultValue="48" className="flex-1 py-2 px-3 bg-gray-700 border border-gray-600 rounded-lg text-white" />
