...
const SupplyPoolPage: React.FC = () => {
  ...
  const regions = [
    ...
    { id: 'AMERS', name: '美洲区', nameEn: 'Americas', count: talents.filter((t) => t.region === 'AMERS').length },
    {
      ...
      count: talents.filter((t) => t.region === 'EMEA').length,
    },
    {
      id: 'CHINA',
      name: '大中华区',
      nameEn: 'Greater China',
      count: talents.filter((t) => t.region === 'CHINA').length,
    },
  ]
  ...
  const filteredTalents = talents.filter((talent) => {
    ...
    const matchesSkills = selectedSkills.length === 0 || selectedSkills.some((s) => talent.skills.includes(s))
              ...
              {filteredTalents.map((talent) => (
                <div
                  ...
                  <div className='flex items-start space-x-4'>
                    {/* Avatar */}
                    <div
                      className={`w-16 h-16 rounded-xl bg-gradient-to-br from-purple-500 to-pink-500 flex items-center justify-center text-white text-xl font-bold flex-shrink-0 ${
                    ...
                    <div className='flex-1 min-w-0'>
                      ...
                      <div className='flex items-start justify-between mb-2'>
                        ...
                        <span
                          className={`text-xs px-2 py-0.5 rounded-full ${getAvailabilityColor(talent.availability)}`}>
                      ...
                      <div className='mt-3 flex items-center space-x-2'>
                        <Zap
                          className={`w-4 h-4 ${
                            talent.aiCollaborationLevel === 'advanced'
                              ? 'text-purple-400'
                              : talent.aiCollaborationLevel === 'intermediate'
                                ? 'text-blue-400'
                                : 'text-gray-400'
                          }`}
                        />
          ...
          <div className='space-y-6'>
            <div className='bg-gradient-to-r from-purple-600/20 to-pink-600/20 rounded-2xl p-8 border border-purple-500/30 text-center'>
              ...
              <p className='text-gray-300 mb-6 max-w-2xl mx-auto'>
                基于多维度分析（技能匹配度、工作历史、AI协同能力、时间区域兼容性等） 为您精准推荐最适合的人才或项目
              </p>
              ...
              {[
                {
                  icon: Award,
                  title: '技能匹配度',
                  description: '基于技能标签和项目经验的精准匹配',
                  color: 'from-blue-500 to-cyan-500',
                },
                {
                  icon: Clock,
                  title: '时间兼容性',
                  description: '跨时区协作的最佳时间窗口计算',
                  color: 'from-purple-500 to-pink-500',
                },
                {
                  icon: Zap,
                  title: 'AI协同能力',
                  description: '基于历史AI协同数据的效率预测',
                  color: 'from-orange-500 to-red-500',
                },
              ].map((factor, index) => (
                <div key={index} className='bg-slate-800/50 rounded-xl p-6 border border-purple-500/20'>
                  <div
                    className={`w-12 h-12 rounded-xl bg-gradient-to-br ${factor.color} flex items-center justify-center mb-4`}>
          ...
          <div className='space-y-6'>
            <div className='grid grid-cols-1 md:grid-cols-2 gap-6'>
              ...
              <div className='bg-slate-800/50 rounded-xl p-6 border border-purple-500/20'>
                ...
                <p className='text-sm text-gray-400 mt-4'>实时汇率转换，智能结算，跨境支付无忧</p>
              </div>
